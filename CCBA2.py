...
width=28
#加载训练集
X_train = np.zeros((n,1,width,width),dtype = np.float64)
Y_train = np.zeros((n,),dtype = np.float64)
path1 = '../2/train/'
objects = os.listdir(path1)
m = 0
for obj in objects:
    m = m + 1
j = 0
for obj in objects:
    for i in tqdm(range(zhang)):
        for k in range(1):
          s=str(i + k)
          #print(obj)
          #print(path + obj + '/%s.jpeg' % s)
          X_train[i + j][k] = cv2.cvtColor(cv2.imread(path1 + obj + '/%s.jpeg' % s), cv2.COLOR_BGR2GRAY)
    j = j + zhang
X_train = np.expand_dims(X_train, axis=4)
for i in range(m):
    Y_train[int((i + 1) * n / m):] =  i + 1
Y_train = np_utils.to_categorical(Y_train, m)
...
n=zhang*2
width=28
X_test = np.zeros((n,1,width,width),dtype = np.float64)
Y_test = np.zeros((n,),dtype = np.float64)
path2 = '../2/test/'
objects = os.listdir(path2)
m = 0
for obj in objects:
    m = m + 1
j = 0
for obj in objects:
    for i in tqdm(range(zhang)):
        for k in range(1):
          s=str(i + k)
          #print(obj)
          #print(path2 + obj + '/%s.jpeg' % s)
          X_test[i + j][k] = cv2.cvtColor(cv2.imread(path2 + obj + '/%s.jpeg' % s), cv2.COLOR_BGR2GRAY)
    j = j + zhang
for i in range(m):
    Y_test[int((i + 1) * n / m):] =  i + 1
...
for conv in convs2:
    print(conv)
merge2 = 0.9*convs2[0]+0.1*convs2[1]
shape=merge2.shape[-1]

output=Dense(2,activation='softmax')(merge2)
...
start = time.time()
h = model.fit(X_train, Y_train,batch_size=64,epochs=5,verbose=1,validation_split=0.1)
end = time.time()
T=end-start
print(T)
